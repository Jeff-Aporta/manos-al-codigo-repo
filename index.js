...
// while (base_preguntas.length > 30) {
//     base_preguntas.pop();
// }
...

{
    ...
    for (let i = 0; i < Object.keys(participantes).length; i++) {
        str += `
            <div class="participante participante-${i}" onclick="responder('.opn-${i}')">
                <style class="var">
                    .participante-${i}{
                        --bg: transparent;
                    }
...
function responder(btn_class) {
    ...
    participanteDOM.querySelector("span").innerHTML = correctas + "/" + realizadas

    participanteDOM.querySelector("progress").setAttribute("value", participantes[p].correctas);
    participanteDOM.querySelector("progress").setAttribute("max", participantes[p].realizadas);
    ...
    if (index >= base_preguntas.length) {
        ...
        for (let i = 0; i < Object.keys(participantes).length; i++) {
            ...
            participanteDOM.querySelector(".var").innerHTML = `
                .participante-${i}{
                    --bg: rgb(${INTERP[0]}, ${INTERP[1]}, ${INTERP[2]})
                }
            `
        }
        return;
